...
async function run() {
    ...
    console.log("\"Government\" -> Send \"Job-Certificate\"     Ledger");
...
// How to do onboarding for government
...
async function onboarding(poolHandle, From, fromWallet, fromDid, to, toWallet, toWalletConfig, toWalletCredentials) {
    ...
    if (!toWallet) {
        ...
        try {
            await indy.createWallet(toWalletConfig, toWalletCredentials) // to = government 
        } catch(e) {
            if(e.message !== "WalletAlreadyExistsError") {
                throw e;
            }
        }
        toWallet = await indy.openWallet(toWalletConfig, toWalletCredentials); //government wallet opened
    }

    console.log(`\"${to}\" > Create and store in Wallet \"${to} ${From}\" DID`);
    let [toFromDid, toFromKey] = await indy.createAndStoreMyDid(toWallet, {});  //getting government did and verkey 
    ...
    //toWallet =  wallet of government means jo wallet request kar raaha ha verkey
...
// finally got key for government . now we can connect with steward 
    ...
    let connectionResponse = JSON.stringify({
        'did': toFromDid,  //goivernment ki did
...
// otherwise phishing hai 
    ...
    // fromToKey = steward ver Key , toWallet = government wallet , toFromDID= government ki did ,  toFromKey = government ki key 
...
async function sendNym(poolHandle, walletHandle, Did, newDid, newKey, role) {  //wallet handle = steward wallet 
